      ...
      if (smart.hasOwnProperty("patient")) {
        ...
        $.when(pt, med).done(function (patient, med) {
          ...
          med.forEach(function (medicationOrder) {
            ...
            if (medicationOrder.medicationCodeableConcept) {
          ...
          checkInteractions(drugPairs)
            .then(function (interactingDrug) {
      ...
      if (medicationOrder.medicationCodeableConcept) {
        ...
        if (
          ...
          if (expirationDate.getTime() < currentDate.getTime()) {
            ...
            var medicationQuantity = medicationOrder.dosageInstruction[0].doseQuantity.value + " " + medicationOrder.dosageInstruction[0].doseQuantity.unit.replace(/{|}/g, "");
            var medicationDirection = medicationOrder.dosageInstruction[0].timing.repeat.frequency + " time(s) per " + medicationOrder.dosageInstruction[0].timing.repeat.period + " day(s)";
            var html = `
              <tr>
                <td>${medicationName}</td>
